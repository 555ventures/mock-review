...
function hasLeadingJsDoc(fullText, node) {
 ...
 * finding.
 */
export function docFindings(files) {
    const findings = [];
    for (const target of files) {
        ...
        if (!hasDoc)
            findings.push({ kind: 'doc', severity: 'error', file: target.file, message: 'missing doc line' });
        if (!hasExamples) {
