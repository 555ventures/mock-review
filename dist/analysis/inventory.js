 ...
 * second file read.
 */
export function inventory(cwd, components, shells, screens) {
    ...
    const usedOnComponents = usedOn(componentAbs.map((c) => c.name), 'components', screens);
    const usedOnShells = usedOn(shellAbs.map((s) => s.name), 'shells', screens);
    const shellNames = new Set(shellAbs.map((s) => s.name));
    const rows = [];
    for (const doc of docs) {
        const isShell = shellNames.has(doc.displayName);
        const kind = isShell ? 'shell' : 'component';
        const props = {};
