...
async function main(argv) {
    ...
    switch (verb) {
        ...
        case 'check': {
            ...
            if (json)
                await printJson(check);
            else
                await writeFlushed(process.stdout, formatCheckText(check));
            return 0;
        }
        case 'sweep': {
            ...
            if (json)
                await printJson(sweep);
            else
                await writeFlushed(process.stdout, formatSweepText(sweep));
            return 0;
        }
        case 'answer': {
