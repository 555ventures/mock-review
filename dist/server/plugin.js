...
import { applyApprovalPatch, applyNotesPatch } from './patches.js';
...
const ENTRY_HTML = '<!doctype html><html lang="en"><head><meta charset="utf-8"/><title>mock-review</title></head>' +
...
const TUNNEL_HEADERS = ['x-forwarded-for', 'forwarded', 'cf-connecting-ip'];
...
function roleOf(req, res, config) {
    if (config.client) {
...
async function configFor(cwd, runner) {
    try {
 ...
 * `home-detail-...png`). */
function screenshotsFor(cwd, name) {
...
function ctxFromState(cwd, state) {
    return {
...
function forceClientThread(thread) {
...
function clientNotesPatch(patch) {
    if (patch.op === 'add') {
...
function clientApprovalPatch(patch) {
...
function readNotesDocOrThrow(cwd) {
    try {
...
function readApprovalDocOrThrow(cwd) {
    try {
...
async function handlePostNotes(req, res, cwd, runner) {
    ...
    await withWriteLock(async () => {
        ...
        if ((appliedPatch.op === 'update' || appliedPatch.op === 'remove') &&
...
async function handlePostApproval(req, res, cwd, runner) {
    ensureDesignFiles(cwd);
    const config = await configFor(cwd, runner);
    const role = roleOf(req, res, config);
    if (role === 'refused') {
...
async function handleEvents(req, res, server, cwd, runner) {
    const config = await configFor(cwd, runner);
