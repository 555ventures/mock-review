...
import { journeyReport } from '../analysis/journeys.js';
...
async function moduleExamplesMap(cwd, runner, files) {
    const map = new Map();
    for (const file of files) {
        const abs = path.join(cwd, file.file);
        try {
            const mod = (await runner.import(abs));
...
function normaliseStepState(exampleKeys, state) {
 ...
 * note.
 */
export async function buildState(cwd, role, runner) {
    ...
    try {
        ...
        if (configResult.finding)
            violations.push(configResult.finding.message);
        const screenResult = await screenReport(cwd, runner, discovered.screens);
        screens = screenResult.screens.map((screen) => ({
            ...screen,
        ...
        journeys = resolved.map((journey, i) => {
            const input = journeyInputs[i];
            const rawSteps = input?.steps ?? journey.steps;
