...
async function exampleStatesFor(serveBaseUrl, screen) {
 ...
 */
export async function checkVerb(cwd, screen, state, lookRequested = screen !== undefined) {
    if (lookRequested) {
        if (screen === undefined)
            fail('--look needs a screen name — remedy: mock-review check --look <screen>');
        const url = await serveUrl(cwd);
        if (url === null)
            fail('--look needs a running serve — remedy: npx mock-review serve');
        const check = await runCheck(cwd);
        const found = check.screens.find((s) => s.name === screen);
        if (!found)
            fail(`--look: unknown screen ${screen}`);
        let states = [];
        if (state === undefined) {
...
export function formatCheckText(check) {
