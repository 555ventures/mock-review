...
/** The one property the package reads off a Vite ssr `DevEnvironment` — a local, never-exported
 * type used only to narrow `env` in `runnerOf`. */
type HasRunner = { runner?: { import?: unknown } }
...
export function runnerOf(env: unknown, ctx: RunnerContext): Runner {
  const candidate = env as HasRunner | undefined
  const runner = candidate?.runner
  if (typeof runner?.import !== 'function') {
    throw new Error(
      `the host's Vite ssr environment has no module runner (vite ${ctx.viteVersion} at ${ctx.root}) — mock-review needs one vite 8 in the host`,
    )
  }
  const runnerImport = runner.import as (absPath: string) => Promise<unknown>
  return {
    import: (absPath: string) => runnerImport.call(runner, absPath),
  }
}
