...
import type { InventoryRow, QueueItem, Sweep } from '../schemas/index.js'
...
function byFileThenId(a: QueueItem, b: QueueItem): number {
  return a.file === b.file ? a.id.localeCompare(b.id) : a.file.localeCompare(b.file)
}

export function sweepVerb(cwd: string): Sweep {
  ...
  const inv: InventoryRow[] = inventory(cwd, discovered.components, discovered.shells, screenSpecifiers)
  ...
  const journeyItems: QueueItem[] = Object.entries(notes.journeys)
  ...
  const noteItems: QueueItem[] = notes.notes
...
export function formatSweepText(sweep: Sweep): string {
  if (sweep.queue.length === 0) return 'queue empty\n'
  ...
  for (const file of fileOrder) {
    ...
    for (const item of byFile.get(file) ?? []) {
      queueLines.push(
        `[${item.kind}] ${item.id} ${item.screen ?? ''}/${item.state ?? ''} line ${item.line} — ${item.last} — reuse: ${item.reuse.join(', ')}`,
      )
    }
  }
