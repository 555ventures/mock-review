...
- [host] `server.environments.ssr` is typed as the base `DevEnvironment`; narrow it by shape with `runnerOf` (`src/analysis/vite-runner.ts`), never with vite's `isRunnableDevEnvironment`, which is an `instanceof` against one copy's class and breaks when the host's vite and the package's vite are different physical copies. — D4 of specs/20260915/01-schemas-cli-and-bare-serve.md, rewritten by D7 of specs/20260916/02-one-vite-per-host.md
...
- [host] A virtual module id that itself carries JSX is not reliably transformed by `@vitejs/plugin-react`, but a virtual id that only *imports* a real `.tsx` file is transformed by extension by the host's own pipeline, even when that file sits under `node_modules`; keep JSX out of virtual modules and let the virtual id's body be a plain `import` statement. — D10 of specs/20260915/03-collapse-the-reviewer-onto-the-host-server.md
...
- [host] Vite's dev `transformRequest` appends a trailing blank line and a `//# sourceMappingURL=` comment to every module it serves, including a plugin's virtual module whose `load` returns a single line, with no per-module opt-out; strip that annotation before asserting a served module's exact line count. — AC-20260915-03-1
- [host] After a frame document reloads, the reviewer's pin overlay is legitimately empty for one double-rAF tick before it re-anchors, and two independent browsing contexts (the reviewer document and the mock iframe) each issue their own request for the same optimized dep with no collapsing; poll for post-reload DOM state and count distinct URLs, not response events, in network-log assertions. — AC-20260915-03-6, AC-20260915-03-2
