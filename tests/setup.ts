...
//
// D6 of specs/20260915/03: the prebuilt-page/frame-entry build this file used to also perform
// (`pageDir`, `frameDir`, `frameEntryPath`, `pageBuildIsStale`, `buildPageAndFrame`) is gone —
// spec 03 deletes the split build entirely; `.test-dist/` is `tsc -p tsconfig.build.json` alone,
// and the reviewer (`src/ui/**`) ships and runs as source, never compiled by this repo's own
// build.
import { execFileSync } from 'node:child_process'
import { chmodSync, existsSync, mkdirSync, lstatSync, readdirSync, rmSync, statSync, symlinkSync, unlinkSync } from 'node:fs'
