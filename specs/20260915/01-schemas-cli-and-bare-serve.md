...
- **AC-20260915-01-8**: WHEN `check --json` runs on the broken fixture host THE SYSTEM SHALL report a finding `{"kind":"render","severity":"error","file":"src/screens/account.tsx","message":"<that state's examples key>: boom"}` (D5's `<state>: <error.message>`; D17) for the state whose example throws, and every other state's absence of a finding → writes tests/cli/check.test.ts
