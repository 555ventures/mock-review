...
# Executed spikes (2026-09-16) — one Vite per host

Run for `specs/20260916/02-one-vite-per-host.md` against vite 8.3.0, Node v24.19.0, in an isolated
worktree unless noted.

| # | Claim | Result |
|---|---|---|
| S1 | `isRunnableDevEnvironment` in vite 8.3.0 is a bare `instanceof` | `grep -n "function isRunnableDevEnvironment" -A2 node_modules/vite/dist/node/chunks/node.js` → `return environment instanceof RunnableDevEnvironment;`. A real `createServer(...).environments.ssr` has `typeof runner.import === 'function'`, `constructor.name === 'RunnableDevEnvironment'`, `this.meta.viteVersion === '8.3.0'` inside a plugin's `configureServer`. |
| S2 | A scratch root under the system temp dir with an empty `node_modules/` does not resolve `vite` from a parent directory | `/tmp/.../spike-b/novite/{package.json,node_modules/}` → `createRequire(root + '/package.json').resolve('vite')` threw `MODULE_NOT_FOUND`, require stack `novite/package.json` only. |
| S3 | `createRequire(<root>/package.json).resolve('vite')` returns the host's `dist/node/index.js` and a dynamic `import()` of it exposes `createServer` | In this repo → `node_modules/vite/dist/node/index.js`; `import(pathToFileURL(p).href)` → `typeof createServer === 'function'`, `version === '8.3.0'`; `exports['.'] === './dist/node/index.js'`; `resolve('vite/package.json')` works. |
| S4 | A physical copy of `node_modules/vite` under a scratch host loads and produces an ssr environment the package's own `isRunnableDevEnvironment` rejects while `runner.import` is a function | `cp -rL node_modules/vite → copy/node_modules/vite` + 134 per-entry symlinks; `import(copy vite)` → `createServer`/`version` present; env from the copy's `createServer` → `repoVite.isRunnableDevEnvironment(env) === false`, `copyVite.isRunnableDevEnvironment(env) === true`, `typeof env.runner.import === 'function'`. |
| S5 | Node 24's `import.meta.resolve` honours a second `parent` argument without a flag | Node v24.19.0, `import.meta.resolve('vite', 'file:///tmp/.../v9/package.json')` → resolved the repo's copy anyway (parent silently ignored, CWD-relative); with `--experimental-import-meta-resolve` → the fake v9 path. `createRequire` is the portable route regardless. |
| S6 | The host's own plugins resolve `vite` from their own install location, not the package's | `@vitejs/plugin-react@6.1.1` `dist/index.js:5` imports `vite/internal`; `@tailwindcss/vite` `dist/index.mjs` imports `vite` directly (`createIdResolver`, `isCSSRequest`). Both resolve from realpath — in a real `npm link` host, that's the same copy the host's own `vite` uses, so the fix leaves exactly one live instance. |
| S7 | The two-copies layout reproduces today's failure only on the host-run plugin-mount path, and the prototype (`loadHostVite` + shape-based `runnerOf`) turns it green with no TypeScript errors | Scratch host = fixture copy with a physical `node_modules/vite` (`differ: true`). Package bin (`check`/`serve`) — **immune today** (server and predicate both from the package's copy): `check --json` exit 0, real config, `findings: []`; `serve` URL + `GET state` real config, `violations: []`. Host-run `node node_modules/vite/bin/vite.js` with `mockReview()` in the host's `vite.config.ts` → `GET /__mock-review/state` → **HTTP 500** `{"message":"the ssr environment is not runnable (no module runner)"}`, every frame in `host/vite.config.ts:1206`. Direct probe: `host isRunnable(ssr): true`, `pkg isRunnable(ssr): false`, `typeof runner.import: function`. Prototype: `tsc -p tsconfig.build.json` exit 0 zero diagnostics; host-run plugin mount → 200 with the real config; bin path unchanged; `loadHostVite.createServer === host copy: true`; no-vite root → stderr warning then a working `createServer`; fake vite 7.1.2 → `mock-review needs vite 8 in the host; found 7.1.2 at …`; `SKIP_BROWSER=1 npx vitest run tests/unit tests/cli` → 17 files, 112 tests passed. Also confirmed `node_modules/.bin/vite` points back at the repo's copy, so `startHostVite` in tests must spawn `host/node_modules/vite/bin/vite.js` directly.
